...
const FONT_INFO __code * __xdata FNT = 0;

void LCD_SetFont(const FONT_INFO __code * fnt)
{
    FNT = fnt;
}

void LCD_DrawChar(uint16_t __generic* x, uint16_t y, uint8_t ch, uint8_t color)
{
    const FONT_CHAR_INFO __code* charInfo = NULL;
...
void LCD_DrawString(uint16_t x, uint16_t y, const char __generic *str, uint8_t color, uint8_t align)
{
...
//    }
    ...
    else // if(align == ALIGN_RIGHT || align == ALIGN_CENTERED)
    {
        ...
        const char __generic *str_temp = str;
        uint16_t x_temp = 0;
        while (*str_temp)
        {
            char ch = *str_temp++;
...
{
	memset_xdata(lcd_buffer, all, sizeof(lcd_buffer));
}
